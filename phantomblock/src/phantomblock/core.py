...
class HostReport:
    ...
    schema: str = "phantomblock-report/v1"
...
def verify_manifest(manifest_path: Path | None) -> list[Finding]:
    ...
    for item in manifest.get("artifacts", []):
        ...
        summary = (
            "Trusted artifact hash matched"
            if actual == expected
            else "Trusted artifact hash mismatch"
        )
        findings.append(
            Finding(
                "firmware_hash",
...
def scan_kernel() -> list[Finding]:
    ...
    if kallsyms.exists() and os.access(kallsyms, os.R_OK):
        ...
        syscall_symbols = re.findall(
            r"^[0-9a-fA-F]+\s+\w\s+(__x64_sys_\w+|sys_call_table)$", text, re.M
        )
        findings.append(
            Finding(
                "syscall_symbols",
                "clean" if syscall_symbols else "medium",
    ...
    suspicious = [
        line
        for line in modules.get("stdout", "").splitlines()
        if re.search(r"rootkit|hook|taint|unsigned module|verification failed", line, re.I)
    ]
    findings.append(
        Finding(
            "kernel_log",
            "high" if suspicious else "clean",
            "Suspicious kernel log indicators found"
            if suspicious
            else "No selected kernel log indicators",
    ...
    if not findings:
        findings.append(
            Finding(
                "oob_interface",
                "clean",
                "No selected management ports reachable on requested hosts",
            )
        )
    return findings
...
def inspect_pcap(path: Path) -> dict[str, Any]:
    ...
    for packet in packets:
        ...
        if sport in watched or dport in watched:
            management_hits.append(
                {"flow": key, "service": watched.get(dport) or watched.get(sport)}
            )
    return {
        "pcap": str(path),
        "packets": len(packets),
...
def collect_report(
    ...
    oob_hosts: list[str] | None = None,
    registry: Any | None = None,
    extension_config: dict[str, Any] | None = None,
) -> HostReport:
    from .extensions import CapabilityContext
    ...
    if registry is not None:
        context = CapabilityContext(
            config=extension_config or {}, evidence={"inventory": inventory}, dry_run=True
        )
        for result in registry.run(context):
            extension_evidence[result.namespace] = result.evidence
            findings.extend(result.findings)
    decision = DecisionPolicy().evaluate(findings)
    return HostReport(
        socket.gethostname(),
